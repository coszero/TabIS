...
from tab_benchmark.utils import default_dump_json, default_load_json, default_load_yaml, check_path
from tab_benchmark.dataset import Dataset, is_valid_dataset
from tab_benchmark.model import model_name2class
from tab_benchmark.metric import MetricToolkit


class TableBenchmarkConfig():
    base_configs = ['exp_path', 'data_base_path', 'data_collection_path']
...
class TableBenchmark():
    ...
    def load_eval_data(self):
        ...
        data_collections = default_load_yaml(self.config.data_collection_path)
        filtered_data_collections = []
        for task_name, datasets in data_collections.items():
            if task_name not in self.config.eval_tasks:
                continue
            print(task_name, datasets)
            for d in datasets:
                if is_valid_dataset(d, self.config.shot, self.config.serialization, self.config.spec_names, self.config.datasets):
                    if d["url"]:
                        url = os.path.join(self.config.data_base_path, d["url"])
                    else:
                        if d.get("suffix") or self.config.suffix != "":
                            suffix = self.config.suffix or d.get("suffixs")
                            json_name = f"{d['serialization']}-{d['shot']}-{suffix}.json"
                        else:
                            json_name = f"{d['serialization']}-{d['shot']}.json"
                        url = os.path.join(self.config.data_base_path, task_name, d['dataset'], json_name)
                    dataset = Dataset(spec_name=d['spec_name'], shot=d['shot'], serialization=d['serialization'], format=d['format'], verbalizer=d['verbalizer'], metric=d['metric'], url=url, other_info=d['other_info'], max_data_num=self.config.max_data_num)
                    filtered_data_collections.append(dataset)
        print(f"valid datasets: {[d.spec_name for d in filtered_data_collections]}")
        self.eval_dataset = filtered_data_collections
    ...
    def load_eval_model(self):
        ...
        for m in self.config.eval_models:
            ...
            if "model_path" in model_config["setting"]:
                # concatenate base_path
                model_config["setting"]["model_path"] = os.path.join(self.config.llm_model_path, model_config["setting"]["model_path"])
    ...
    def eval(self):
        ...
        for dataset in self.eval_dataset:
            for model in self.models:
                print(f"evaluate model={model.spec_name} on dataset={dataset.spec_name}")
...
class EvalTask():
    ...
    def eval_model_on_dataset(self, dump_base_path, skip_eval=False, gpu_devices=None, pool_num=1, script_path=None, eval_group_key=None):
        """
    ...
    def get_model_output_gpu(self, verbalizer, gpu_devices, script_path, run_dump_path):
        ...
        if os.path.exists(all_reply_info_path):
            ...
            all_reply_info = self.model.forward_all_samples(self._dataset.dataset, gpu_devices, script_path, 
    ...
    def get_model_output_cpu(self, verbalizer, pool_num, run_dump_path):
        ...
        if pool_num == 1:
            if os.path.exists(res_dump_path):
                ...
                print(f"start from: index {done_num}")
            else:
                all_reply_info = []
                done_num = 0
            ...
            if os.path.exists(output_path):
                ...
                print(f"working on {len(data_pairs)} remaining samples under {output_path}.")
            ...
            # pool = Pool(pool_num)
            # pool.map(func, data_pairs)
            # pool.close()
            # pool.join()
            # end_time = time.time()
            # print(f"time elapsed: {(end_time-start_time)/60} mins.")
            sorted_fn = sorted(os.listdir(output_path), key=lambda x: int(x.split('.')[0]))
            all_reply_info = []
            for fn in sorted_fn:
